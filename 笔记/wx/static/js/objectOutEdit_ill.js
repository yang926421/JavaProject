...
$(function () {
    ...
    $("#commit").on("click", function () {
        if (uploadSave(uploadData, type)) {
            window.location.href = 'objectDisdetail.html';
        }
        img = null;
    })
    ...
    $uploaderInput.on("change", function (e) {
        ...
        if (filesNum >= pubAcceptPicNum) {
            mui.toast("最多只能上传" + pubAcceptPicNum + "张图片");
            return;
        }
        ...
        for (var i = 0, len = files.length; i < len; ++i) {
            ...
            //$("#oldImg").remove();
            $uploaderFiles.append($(tmpl.replace('#url#', src)));  //因为只要一个图片，所以这里是html（）而不是append（）
...
function uploadSave(basestr, type) {
    ...
    formdata.append("APPLYER", $("#APPLYER").val());
    formdata.append("TEL", $("#TEL").val());
    formdata.append("CERNO", $("#CERNO").val());
    formdata.append("REASONS", $("#REASONS").val());
    formdata.append("CREATEDUSER", $("#CREATEDUSER").val());
    formdata.append("ILLID", $("#ILLID").val());
    formdata.append("outId", $("#outId").val());

    xhr.open('post', httpUrl + '/e/ill/object/out/save.json');
    //创建遮罩加载点
